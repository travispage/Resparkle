                ...
                <td class="product">
                    ...
                    <dl class="meta">
                        ...
                        <?php if (!empty($item['sku'])) : ?>
                            <dt class="sku"><?php _e('SKU:', 'wpo_wcpdf'); ?></dt>
                            <dd class="sku"><?php echo $item['sku']; ?></dd><?php endif; ?>
                        <?php if (!empty($item['weight'])) : ?>
                            <dt class="weight"><?php _e('Weight:', 'wpo_wcpdf'); ?></dt>
...
<?php do_action('wpo_wcpdf_after_order_details', $wpo_wcpdf->export->template_type, $wpo_wcpdf->export->order); ?>
    ...
    <div class="customer-notes">
        ...
        <?php
        // Check to see if the order is the first in a subscription

        // First, connect to the database
        $servername = "localhost";
        $username = "resparkl_db2015";
        $password = "Ra1ndr0p";
        $dbname = "resparkl_2015";
        ...
        $oid = $wpo_wcpdf->get_order_number();

        $sql = "SELECT * FROM wp_posts WHERE post_parent = $oid";
        $result = $conn->query($sql);

        if ($result->num_rows > 0) {
            // output data of each row
            while ($row = $result->fetch_assoc()) {
                echo '<strong>* First box for this subscriber</strong>';
            }
        }
        $conn->close();
        ?>
