...
wp_register_script( 'chosen', get_template_directory_uri() . '../../../plugins/woocommerce/assets/js/chosen/chosen.jquery.min.js', array( 'jquery' ) );
wp_enqueue_script( 'chosen' );

wp_register_script( 'ajax_chosen', get_template_directory_uri() . '../../../plugins/woocommerce/assets/js/chosen/ajax-chosen.jquery.min.js', array( 'jquery' ) );
wp_enqueue_script( 'ajax_chosen' );
...
/*
*	Replace woocommerce default stars system with ours, to show stars even on no reviews.
 */
add_filter('woocommerce_product_get_rating_html', 'update_rating_stars');
if ( ! function_exists( 'update_rating_stars' ) ) {
	function update_rating_stars( $rating = null ) {
		global $product;
...
function woocommerce_template_loop_stock() {
	global $product;
 	if ( ! $product->managing_stock() && ! $product->is_in_stock() ) {
 		echo '<p class="stock out-of-stock">' . __( 'Coming Soon', 'woothemes' ) . '</p>';
 	}
}
...
/* Adjust Add to cart and price layout on product detail */
...
/* Remove category/meta information from Product Detail */
...
add_action( 'woocommerce_after_single_product', 'woo_upsell_display', 3 );
...
function superstore_custom_cart_button_text( $text, $product ) {
    ...
    $result = __('ADD TO CART', 'woothemes');
