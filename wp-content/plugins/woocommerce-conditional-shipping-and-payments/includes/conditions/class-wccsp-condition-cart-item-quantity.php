...
class WC_CSP_Condition_Cart_Item_Quantity extends WC_CSP_Condition {
	...
	public function __construct() {
		...
		$this->title                         = __( 'Quantity', 'woocommerce-conditional-shipping-and-payments' );
	...
	public function get_condition_resolution( $data, $args ) {
		...
		if ( $data[ 'modifier' ] === 'min' && $data[ 'value' ] <= $product_quantity ) {
			return sprintf( __( 'decrease the quantity of &quot;%1$s&quot; to a value lower than %2$s', 'woocommerce-conditional-shipping-and-payments' ), $cart_item_data[ 'data' ]->get_title(), $data[ 'value' ] );
		}

		if ( $data[ 'modifier' ] === 'max' && $data[ 'value' ] > $product_quantity ) {
			return sprintf( __( 'increase the quantity of &quot;%1$s&quot; to at least %2$s', 'woocommerce-conditional-shipping-and-payments' ), $cart_item_data[ 'data' ]->get_title(), $data[ 'value' ] );
		}
