...
if( !class_exists( 'EDD_SL_Plugin_Updater' ) ) {
	require_once plugin_dir_path( dirname( __FILE__ ) ) . 'edd_licence/EDD_SL_Plugin_Updater.php';
}
...
class EDD_WPGens_RAF_Licence {
	 ...
	 * @since 		1.1.0
	 */
	function gens_raf_auto_update() {

		$license_key = trim( get_option( 'gens_raf_license_key' ) );

		// setup the updater
		$edd_updater = new EDD_SL_Plugin_Updater( GENS_RAF_STORE_URL, GENS_RAF_PLUGIN_PATH.'/gens-raf.php', array(
				'version' 	=> '1.2.1', // current version - change on every plugin change
				'license' 	=> $license_key,
				'item_name' => GENS_RAF_ITEM_NAME,
				'author' 	=> 'WPGens',
				'beta'		=> false
			)
		);
	...
	public function gens_raf_activate_licence() {
		...
		if( isset( $_POST['gens_raf_license_activate'] ) ) {
			...
			$api_params = array(
				...
				'license'    => $license,
				'item_name'  => urlencode( GENS_RAF_ITEM_NAME ), // the name of our product in EDD
				'url'        => home_url()
			);

			// Call the custom API.
			$response = wp_remote_post( GENS_RAF_STORE_URL, array( 'timeout' => 15, 'sslverify' => false, 'body' => $api_params ) );
				...
				if ( false === $license_data->success ) {
					...
					switch( $license_data->error ) {
						...
						case 'item_name_mismatch' :
							...
							$message = sprintf( __( 'This appears to be an invalid license key for %s.' ), GENS_RAF_ITEM_NAME );
							break;
			...
			if ( ! empty( $message ) ) {
				$base_url = admin_url( 'admin.php?page=' . GENS_RAF_PLUGIN_LICENSE_PAGE );
			...
			wp_redirect( admin_url( 'admin.php?page=' . GENS_RAF_PLUGIN_LICENSE_PAGE ) );
			exit();
		}
	}
