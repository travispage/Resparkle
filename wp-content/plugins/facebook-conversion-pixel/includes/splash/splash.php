...
function fca_pc_render_splash_page() {
	...
	echo '<form method="post" action="' . admin_url( '/admin.php?page=pixel-cat-splash' ) . '">';
		...
		echo '<div id="fca-splash">';
			echo '<h1>' . __( 'Welcome to Pixel Cat', 'pixel-cat' ) . '</h1>';
			
			echo '<div id="fca-splash-main" class="fca-splash-box">';
				echo '<p id="fca-splash-main-text">' .  sprintf ( __( 'In order to enjoy all our features and functionality, Pixel Cat needs to connect %1$s your user, %2$s at %3$s, to <strong>api.fatcatapps.com</strong>.', 'pixel-cat' ), '<br>', '<strong>' . $name . '</strong>', '<strong>' . $website . '</strong>'  ) . '</p>';
				echo "<button type='submit' id='fca-pc-submit-btn' class='fca-pc-button button button-primary' name='fca-pc-submit-optin' >" . __( 'Connect Pixel Cat', 'pixel-cat') . "</button><br>";
				echo "<button type='submit' id='fca-pc-optout-btn' name='fca-pc-submit-optout' >" . __( 'Skip This Step', 'pixel-cat') . "</button>";
			echo '</div>';
			
			echo '<div id="fca-splash-permissions" class="fca-splash-box">';
				echo '<a id="fca-splash-permissions-toggle" href="#" >' . __( 'What permission is being granted?', 'pixel-cat' ) . '</a>';
				echo '<div id="fca-splash-permissions-dropdown" style="display: none;">';
					echo '<h3>' .  __( 'Your Website Info', 'pixel-cat' ) . '</h3>';
					echo '<p>' .  __( 'Your URL, WordPress version, plugins & themes. This data lets us make sure this plugin always stays compatible with the most popular plugins and themes.', 'pixel-cat' ) . '</p>';
					
					echo '<h3>' .  __( 'Your Info', 'pixel-cat' ) . '</h3>';
					echo '<p>' .  __( 'Your name and email.', 'pixel-cat' ) . '</p>';
					
					echo '<h3>' .  __( 'Plugin Usage', 'pixel-cat' ) . '</h3>';
					echo '<p>' .  __( "How you use this plugin's features and settings. This is limited to usage data, and does not include your Facebook Pixel Data. This data helps us learn which features are most popular, so we can improve the plugin further.", 'pixel-cat' ) . '</p>';				
				echo '</div>';
			echo '</div>';
	...
	echo '<div id="fca-splash-footer">';
		echo '<a target="_blank" href="https://fatcatapps.com/legal/terms-service/">' . _x( 'Terms', 'as in terms and conditions', 'pixel-cat' ) . '</a> | <a target="_blank" href="https://fatcatapps.com/legal/privacy-policy/">' . _x( 'Privacy', 'as in privacy policy', 'pixel-cat' ) . '</a>';
	echo '</div>';
}
