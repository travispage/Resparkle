...
jQuery(document).ready(function($){
	...
	$('#fca-pc-helptext, .fca_pc_error_tip').tooltipster( { trigger: 'custom', timer: 6000, maxWidth: 350, theme: ['tooltipster-borderless', 'tooltipster-pixel-cat'] } )
	$('.fca_pc_tooltip, .fca_pc_event_tooltip').tooltipster( { contentAsHTML: true, theme: ['tooltipster-borderless', 'tooltipster-pixel-cat'] } )
	...
	$('#fca_pc_new_event').click(function(){
		...
		if ( typeof fcaPcPremiumData === 'undefined' ) {
			$('.fca-pc-content_name').val('')
			$('.fca-pc-content_type').val('')
			$('.fca-pc-content_ids').val('')
			$('.fca-pc-content_category').val('')
		}
	...
	$('#fca-pc-event-save').click(function(){
		...
		//DELAY
		if ( time_delay_events.indexOf( fbEvent.event ) !== -1 || $('#fca-pc-modal-event-input').val() === 'custom' ) {
	...
	$('#fca-pc-modal-event-input').change(function(){
		...
		//MAYBE SHOW THE TIME DELAY OPTION
		if ( time_delay_events.indexOf( $(this).val() ) !== -1 && $('#fca-pc-modal-trigger-type-input').val() === 'post' ) {
			$('#fca-pc-modal-delay-input').closest('tr').show()
			$('#fca_pc_tooltip_viewcontent').show()
		} else {
			$('#fca-pc-modal-delay-input').closest('tr').hide()
	...
	//TURN OFF INPUTS IF WOO IS NOT INSTALLED
	$('.fca-pc-integration-disabled input').each(function(){
	...
	function attach_row_actions() {
		...
		$('.fca_delete_icon_confirm').click( function(){
		...
		$('.fca_delete_icon_cancel').click( function(){
			$(this).hide()
		...
		$('.fca_pc_event_row').click( function( click ){
			...
			if ( $(click.target).hasClass('fca_delete_icon') === false ) {
	...
	//HIDE SETUP NOTICE IF PIXEL IS FILLED IN
	if ( $('.fca-pc-id').val() !== '' ) {
