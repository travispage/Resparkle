<?php
/**
 * WC_Gateway_Pin_Payments_Subscriptions class.
 * 
 * @extends WC_Gateway_Pin_Payments
 */
class WC_Gateway_Pin_Payments_Subscriptions extends WC_Gateway_Pin_Payments {
	...
	function process_subscription_payment( $order = '', $amount = 0 ) {
		...
		if (in_array(get_woocommerce_currency(),array('BIF','CLP','DJF','GNF','JPY','KMF','KRW','MGA','PYG','RWF','VND','VUV','XAF','XOF','XPF'))) {
			$damount = $order->get_total();
		} else {
			$damount = number_format( (float)$order->get_total() * 100, 0, '.', '' );
		}
	...
	protected function save_subscription_meta( $order_id, $customer_id ) {
		...
		update_post_meta( $order_id, '_pin_customer_token', $customer_id );
		...
		foreach( wcs_get_subscriptions_for_order( $order_id ) as $subscription ) {
			update_post_meta( $subscription->id, '_pin_customer_token', $customer_id );
		}
	}
