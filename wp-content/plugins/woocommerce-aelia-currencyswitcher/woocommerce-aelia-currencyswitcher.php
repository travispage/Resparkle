...
class WC_Aelia_CurrencySwitcher implements IWC_Aelia_CurrencySwitcher {
	 ...
	 * but it should no longer be required.
	 *
	 ...
	 *
	 * @deprecated since 3.6.29.150424
	 */
	...
	public function wc_coupon_loaded($coupon) {
		...
		if(in_array($coupon->discount_type, $coupon_types_to_convert)) {
			$coupon->coupon_amount = $this->convert($coupon->coupon_amount,
																							$this->settings_controller()->base_currency(),
		...
		if((int)$coupon->minimum_amount > 0) {
			$coupon->minimum_amount = $this->convert($coupon->minimum_amount,
																							 $this->settings_controller()->base_currency(),
																							 $this->get_selected_currency());
			$coupon->maximum_amount = $this->convert($coupon->maximum_amount,
																							 $this->settings_controller()->base_currency(),
																							 $this->get_selected_currency());
		}
		return $coupon;
	...
	public function woocommerce_loaded() {
		...
		// data is loaded
		if(!$this->visitor_is_bot()) {
			global $woocommerce;
			// Check if a session was already started, before trying to start one
			if(!method_exists($woocommerce->session, 'has_session') ||
