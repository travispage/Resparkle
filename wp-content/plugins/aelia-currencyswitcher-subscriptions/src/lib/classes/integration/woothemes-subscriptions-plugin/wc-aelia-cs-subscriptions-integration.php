...
class Subscriptions_Integration {
	...
	protected function convert_to_currency(WC_Product $product, $currency,
		...
		$product->subscription_price = get_value($currency,
																						 ...
																						 $this->currencyprices_manager()->convert_product_price_from_base($product->subscription_price, $currency, $product_base_currency, $product, 'regular_price'));
		$product->regular_price = $product->subscription_price;
		$product->sale_price = get_value($currency,
																		 $product_sale_prices_in_currency,
																		 $this->currencyprices_manager()->convert_product_price_from_base($product->sale_price, $currency, $product_base_currency, $product, 'sale_price'));
		$product->subscription_sign_up_fee = get_value($currency,
																									 $product_signup_prices_in_currency,
																									 $this->currencyprices_manager()->convert_product_price_from_base($product->subscription_sign_up_fee, $currency, $product_base_currency, $product, 'signup_fee'));
