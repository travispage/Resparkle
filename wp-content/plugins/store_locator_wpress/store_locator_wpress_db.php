...
class Store_locator_wpress_db {
		...
		*/
		...
		if( $postal != 'undefined' ) {
			...
			if(is_numeric($postal)) {
				// $sql .= "AND s.postal_code = ".$postal;
				if($radius_id!='') $sql .= " HAVING distance<='".$radius_id."' OR s.postal_code = ".$postal;
			} else {
				$sql .= "AND s.name LIKE  '%".$postal."%' OR s.address LIKE '%".$postal."%'";
			}
	...
	function update_store($criteria) {
		...
		name='".$criteria['name']."', postal_code='".$criteria['postal_code']."', logo='".$criteria['logo']."', url='".$criteria['url']."', 
	...
	function add_store($criteria) {
		...
		(user_id, post_id, category_id, name, postal_code, logo, address, lat, lng, url, description, tel, email, created) 
		VALUES ('".$criteria['user_id']."', '".$criteria['post_id']."', '".$criteria['category_id']."', '".$criteria['name']."', '".$criteria['postal_code']."', '".$criteria['logo']."', '".$criteria['address']."', '".$criteria['lat']."', '".$criteria['lng']."', 
