...
class Store_locator_wpress_display {
	...
	function get_store_details_display($stores) {
		...
		$display .= '<div id="map" style="overflow: hidden; width:'.$width.'; height:'.$height.'; max-width: none;"></div>';
	...
	function display_stores_list($stores,$criteria=array()) {
		...
		for($i=0; $i<count($stores); $i++) {
			...
			if(count($stores)>1) $content .= '<a href="#" data-lat="'.$stores[$i]['lat'].'" data-lng="'.$stores[$i]['lng'].'" class="store">';
			else $content .= '<a href="#" data-lat="'.$stores[$i]['lat'].'" data-lng="'.$stores[$i]['lng'].'" class="store single">';
			/*
			...
			*/
			...
			$content .= '<strong>'.$stores[$i]['name'].'</strong>';
	...
	*/
	...
	function displayAddressSearchBox($criteria=array()) {
		...
		$display = '<h1>'.$GLOBALS['store_locator_lang']['search_by_address'].'</h1>';
		
		$display .= '<form method="GET">';
			$display .= '<input type="text" id="store_wpress_address" name="store_wpress_address" style="width:440px;" value="'.$_GET['address'].'" placeholder="Postal Code (Australia Only)"/>';
